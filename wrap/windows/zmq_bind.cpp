...
class NativeContextGuard final {
  public:
    explicit NativeContextGuard(void *native) noexcept : native_(native) {}
    ~NativeContextGuard() noexcept {
        if (native_ == nullptr) {
            return;
        }
        int result;
    ...
    void *release() noexcept { return std::exchange(native_, nullptr); }

  private:
    void *native_;
};

class NativeSocketGuard final {
  public:
    explicit NativeSocketGuard(void *native) noexcept : native_(native) {}
...
class ZmqMessageGuard final {
  public:
    ZmqMessageGuard() noexcept : initialized_(zmq_msg_init(&message_) == 0) {}
    ~ZmqMessageGuard() noexcept {
        if (initialized_) {
            (void) zmq_msg_close(&message_);
        }
    }

    ZmqMessageGuard(const ZmqMessageGuard &) = delete;
    ZmqMessageGuard &operator=(const ZmqMessageGuard &) = delete;
...
zmq_handle_t context_create_impl(const int io_threads) {
    if (io_threads <= 0) {
        fail();
        return 0;
    }

    NativeContextGuard native(zmq_ctx_new());
...
int context_destroy_impl(const zmq_handle_t context) {
...

    {
        ...
        // No operation below this point allocates memory.
...
zmq_handle_t socket_create_impl(const zmq_handle_t context, const int socket_type) {
    ...
    NativeSocketGuard native(zmq_socket(context_state->native, socket_type));
    if (native.get() == nullptr) {
        g_last_error = zmq_errno();
        return 0;
    }
...
int socket_close_impl(const zmq_handle_t socket) {
...
int bind_impl(const zmq_handle_t socket, const char *endpoint) {
...
int connect_impl(const zmq_handle_t socket, const char *endpoint) {
    std::shared_ptr<SocketState> state = find_socket(socket);
    if (!state || endpoint == nullptr) {
        return fail();
    }
...
int send_impl(const zmq_handle_t socket,
...
int send_int_array_impl(const zmq_handle_t socket,
                        const int *data,
                        const int count,
                        const int flags) {
...
int send_double_array_impl(const zmq_handle_t socket,
                           const double *data,
                           const int count,
                           const int flags) {
...
int receive_impl(const zmq_handle_t socket,
...
int set_option_int_impl(const zmq_handle_t socket, const int option, const int value) {
...
int set_option_int64_impl(const zmq_handle_t socket,
                          const int option,
                          const int64_t value) {
    if (!is_int64_option(option)) {
        return fail();
...
int set_option_bytes_impl(const zmq_handle_t socket,
                          const int option,
...
int get_option_int_impl(const zmq_handle_t socket, const int option, int *value) {
...
int get_option_int64_impl(const zmq_handle_t socket, const int option, int64_t *value) {
    if (value == nullptr || !is_int64_option(option)) {
        return fail();
    }
    std::shared_ptr<SocketState> state = find_socket(socket);
...
int poll_impl(const zmq_handle_t socket, const int events, const int timeout_ms) {
    std::shared_ptr<SocketState> state = find_socket(socket);
    if (!state) {
        return fail();
    }
...
int error_text_impl(const int error_code, unsigned char *buffer, const int capacity) {
    if (capacity < 0 || (capacity > 0 && buffer == nullptr)) {
        return fail();
    }
...
int ZMQ_BIND_CALL zmqb_send(const zmq_handle_t socket,
    ...
    return invoke_int([&] { return send_impl(socket, data, length, flags); });
}
...
int ZMQ_BIND_CALL zmqb_send_int_array(const zmq_handle_t socket,
                                      ...
                                      const int count,
                                      const int flags) noexcept {
    return invoke_int([&] { return send_int_array_impl(socket, data, count, flags); });
}
...
int ZMQ_BIND_CALL zmqb_send_double_array(const zmq_handle_t socket,
                                         ...
                                         const int count,
                                         const int flags) noexcept {
    return invoke_int([&] { return send_double_array_impl(socket, data, count, flags); });
}
...
int ZMQ_BIND_CALL zmqb_receive(const zmq_handle_t socket,
                               ...
                               const int capacity,
                               const int flags) noexcept {
    return invoke_int([&] { return receive_impl(socket, buffer, capacity, flags); });
}

int ZMQ_BIND_CALL zmqb_set_option_int(const zmq_handle_t socket,
                                      const int option,
                                      const int value) noexcept {
    return invoke_int([&] { return set_option_int_impl(socket, option, value); });
}

int ZMQ_BIND_CALL zmqb_set_option_int64(const zmq_handle_t socket,
                                        const int option,
                                        const int64_t value) noexcept {
    return invoke_int([&] { return set_option_int64_impl(socket, option, value); });
}
...
int ZMQ_BIND_CALL zmqb_set_option_bytes(const zmq_handle_t socket,
    ...
    return invoke_int([&] { return set_option_bytes_impl(socket, option, value, length); });
}

int ZMQ_BIND_CALL zmqb_get_option_int(const zmq_handle_t socket,
                                      const int option,
                                      int *value) noexcept {
    return invoke_int([&] { return get_option_int_impl(socket, option, value); });
}

int ZMQ_BIND_CALL zmqb_get_option_int64(const zmq_handle_t socket,
                                        const int option,
                                        int64_t *value) noexcept {
    return invoke_int([&] { return get_option_int64_impl(socket, option, value); });
}

int ZMQ_BIND_CALL zmqb_poll(const zmq_handle_t socket,
                            const int events,
                            const int timeout_ms) noexcept {
    return invoke_int([&] { return poll_impl(socket, events, timeout_ms); });
}
...
int ZMQ_BIND_CALL zmqb_error_text(const int error_code,
    ...
    return invoke_int([&] { return error_text_impl(error_code, buffer, capacity); });
}
